...
router.post('/create', async function (req, res) {
  try {
    ...
    const total = await calculateTotalValue(products);
    await updateTotalSaleValue(salesId, total, db);
...
async function checkStock(products) {
  for (const product of products) {
    try {
      const response = await axios.get(`http://localhost:3000/stock/${product.product_id}`)
...
async function createSale(client_id, db) {
  return new Promise((resolve, reject) => {
    const salesQuery = 'INSERT INTO sales (client_id, total) VALUES (?, ?)'
...
async function updateStock(productId, quantity) {
  try {
    ...
    const response = await axios.put(`http://localhost:3000/stock/lower/${productId}`, requestBody)
...
async function updateTotalSaleValue(salesId, total, db) {
  return new Promise((resolve, reject) => {
    const sqlUpdateSale = 'UPDATE sales SET total = ? WHERE id = ?';

    db.query(sqlUpdateSale, [total, salesId], (err, results) => {
      if (err) {
